...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (userType == "1" || userType == "2" || userType == "7" || userType == "8") {
                _all_university = _all_university
            ...
            app = new Vue({
                ...
                methods: {
                    ...
                    excelExport: function() {
                        ...
                        let tmpEmpty = Array.from("".repeat(4));
                        wsData.push([]);
                        var titleRow = ["bigtitle-tw", 1, "목적 외 활용 금지 서약"];
                        titleRow.concat(tmpEmpty);
                        wsData.push(titleRow);

                        let desc = "본 시스템은 재학생의 진로지도 및 대학별 진로지도 관련 정책 수립에 도움을 드리기 위한 목적으로 제공되고 있습니다."
                        titleRow = ["bigtitle-tw-agree", 1, desc];
                        titleRow.concat(tmpEmpty);
                        wsData.push(titleRow);
                        ...
                        var wanted_bookjoblcd = _this.selectedBookJoblcd == null ? '' : _this.selectedBookJoblcd.name;
                        ...
                        var _selectedAllValue = [{
                            ...
                            },
                            {
                                seq: 3,
                                name: '해외연수 여부',
                                value: _this.oversea === 'A' ? 'Y' : 'N'
                            },
                            {
                                seq: 3,
                                name: '현장경험 여부',
                                value: _this.internship === '1' ? 'Y' : 'N'
                            },
                            {
                                seq: 4,
                                name: '총 자격증 개수',
                        ...
                        wsData.push(['data', _this.chart1.length, ..._this.chart1.map(x => x.y)]);
                        wsData.push(rowEmpty);

                        wsData.push(getSubtitle('▶ 산업 분류별 취업 가능성(%)'));
                        wsData.push(['header', _this.chart2.length, ..._this.chart2.map(x => x.name)]);
                        wsData.push(['data', _this.chart2.length, ..._this.chart2.map(x => x.y)]);
                        wsData.push(rowEmpty);
                        ...
                        wsData.push(['data', _this.chart4.length, ..._this.chart4.map(x => x.y)]);
                        wsData.push(rowEmpty);

                        wsData.push(getSubtitle('▶ 직무 유형별 취업 가능성(%)'));
                        wsData.push(['header', _this.chart5.length, ..._this.chart5.map(x => x.name)]);
                        wsData.push(['data', _this.chart5.length, ..._this.chart5.map(x => x.y)]);
                        wsData.push(rowEmpty);

                        if (_this.wantedResult) {
                            wsData.push(getSubtitle('▶ 학점/TOEIC/자격증 영향도(%)'));
                            wsData.push(['header', 6, '구분', '희망 기업유형', '희망 산업분류', '희망 취업소요기간', '희망 급여', '희망 직무유형']);
                            wsData.push(['header', 6, '', wanted_comtype, wanted_comind, wanted_duration, wanted_salary, wanted_bookjoblcd]);
                            ...
                            if (Object.keys(nowData.wanted_bookjoblcd).length > 0) {
                                avgarr[4] = nowData.wanted_bookjoblcd.expected ? nowData.wanted_bookjoblcd.expected : '';
                                scorear[4] = nowData.wanted_bookjoblcd.shap_gradescore ? nowData.wanted_bookjoblcd.shap_gradescore + nowData.wanted_bookjoblcd.expected : nowData.wanted_bookjoblcd.expected;
                                toeicarr[4] = nowData.wanted_bookjoblcd.shap_toeic ? nowData.wanted_bookjoblcd.shap_toeic + nowData.wanted_bookjoblcd.expected : nowData.wanted_bookjoblcd.expected;
                                licarr[4] = nowData.wanted_bookjoblcd.shap_license ? nowData.wanted_bookjoblcd.shap_license + nowData.wanted_bookjoblcd.expected : nowData.wanted_bookjoblcd.expected;
                            }
                            wsData.push(['data', 6, '학과 평균 취업가능성'].concat(avgarr));
                            wsData.push(['data', 6, '학점 반영 취업가능성'].concat(scorear));
                            if (toeic > 0) {
                                wsData.push(['data', 6, 'TOEIC 반영 취업가능성'].concat(toeicarr));
                            }
                            if (lictotcnt > 0) {
                                wsData.push(['data', 6, '자격증 반영 취업가능성'].concat(licarr));
                            }
                        }
                        var _sheet = json_to_sheet(wsData);
                        var ws = _sheet.data;
                    ...
                    validation: function() {
                        ...
                        var internship = this.internship;
                    ...
                    moveSearchStatus() {
                        ...
                        if (confirm(confirmMsg)) {
                            window.location.href = 'status.jsp?SCHCD=' + gSchcd + '&ORGCD=' + gOrgcd + '&USERID=' + userid + '&USERGRPTYPE='+ userType +'&COMTYPE=' + selectComtype + '&COMIND=' + selectComind;
                        }
                    },
