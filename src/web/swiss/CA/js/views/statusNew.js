                ...
                methods: {
                    ...
                    excelExport: function () {
                        ...
                        if (vm.isSelectedCorp) {
                            ...
                            if (vm.selectedCompanySizeList.length > 0) {
                                vm.selectedCompanySizeList.forEach((item) => {
                                    var dateObj = _.find(vm.companySizes, { value: item });
                            ...
                            if (vm.selectedCompanyPersonnelSizeList.length > 0) {
                                vm.selectedCompanyPersonnelSizeList.forEach((item) => {
                                    tmpItem = item.join(",");
                                    var dateObj = _.find(vm.companyPersonnelSizes, { value: tmpItem });
                        ...
                        var getTitle = (num) => {
                            ...
                            if (vm.isSelectedCorp) {
                                var searchRow1 = ["search3", num, `1. 통계 기준: ${vm.selectedYear}`, "", ""];
                        ...
                        wsData.push([
                            "data",
                            4,
                            vm.analysis_university_origin.name + " (" + vm.analysis_university_origin.value + ")",
                            vm.analysis_university_area.name + " (" + vm.analysis_university_area.value + ")",
                            vm.analysis_university_kind.name + " (" + vm.analysis_university_kind.value + "%)",
                            vm.analysis_university_fund.name + " (" + vm.analysis_university_fund.value + "%)",
                        ]);
                        wsData.push(rowEmpty);
                        wsData.push(getSubtitle("▶전공(1~3순위)"));
                        wsData.push(["header", 3, "순위", "학과(계열)", "소계열"]);
                        for (var i = 0; i < vm.major_rank.length; i++) {
                            tmpMajorRank = vm.major_rank[i].name ? vm.major_rank[i].name + " (" + vm.major_rank[i].value + ")" : "";
                            tmpMajorFieldRank = vm.major_field_rank[i].name ? vm.major_field_rank[i].name + " (" + vm.major_field_rank[i].value + ")" : "";
                     ...
                     */
                    changeYear: function () {
                        let url = url_getStatUICode(this.selectedYear);
                        ...
                        $.ajax({
                            ...
                            dataType: "text",
                            success: function (data) {
                                vm.responseData = JSON.parse(data);
                                vm.handleInit();
                            },
                        });
                    },
                    ...
                    selectRelatedCompany: function() {
                        this.companySearch = this.selectedRelatedCompany.text
                        this.selectedCompany = this.selectedRelatedCompany;
                    ...
                    handleSearchCompanyListSubmit: function () {
                        ...
                        if (this.selectedCompanyArea == null &&
                            ...
                            if (this.selectedCompanyArea) {
                                data["area"] = this.selectedCompanyArea;
                            }
                            ...
                            success: function (response) {
                                ...
                                if (res.count == undefined || res.count == 0) {
                                    ...
                                    setRenderPieChartBySize(
                                        "fund",
                                        _.map(_.keys(university_fund_trans), function (key, index) {
                                            if (index == 0) {
                                    ...
                                    setRenderPieChartBySize(
                                        "kind",
                                        _.map(_.keys(university_kind_trans), function (key, index) {
                                            if (index == 0) {
                                    ...
                                    setRenderMultipleAxisChart("toeic_chart", res.toeic, Highcharts, 200, toeicmin, 990, 50.0);
                                    ...
                                    setRenderMultipleAxisChart("grade_chart", res.gradescore, Highcharts, 10, scoremin, 100, 5.0);
                                    ...
                                    //인턴
                                    setRenderHalfDounutChart(
                                        "internChart",
                                        _.map(_.keys(res.internship), function (key) {
                                    ...
                                    setRenderHalfDounutChart(
                                        "transferChart",
                                        _.map(_.keys(res.transfer), function (key) {
                                    ...
                                    setRenderHalfDounutChart(
                                        "teachChart",
                                        _.map(_.keys(res.teach), function (key) {
                    ...
                    companySearchOnSubmit: function () {
                        const vm = this;
                        let searchText = vm.companySearch;
                        ...
                        if (searchText.length > 0) {
                            ...
                            if (comres.length == 0) {
                                ...
                                var tmpYearRes = comres.filter((item) => item.value == vm.selectedYear);
                    ...
                    renderGridAndChart: function (val, id) {
                        ...
                        if (id == "origin") {
                            if (val) {
                                ...
                                var top3Data = this.university_origin_tab_list;
                                if (top3Data.length > 3) {
                                    top3Data = this.university_origin_tab_list.slice(0, 3);
                                }
                            ...
                            if (val) {
                                ...
                                vm.university_dept_tab_list = vm.filterBySelectedCategory(selected[vm.category_deptcol_selected]);
                                vm.university_dept_tab_list_length = vm.university_dept_tab_list.length;

                                var top3Data = vm.university_dept_tab_list;
                                if (top3Data.length > 3) {
                                    top3Data = this.university_dept_tab_list.slice(0, 3);
                                }
                                ...
                                //학력 pie chart render
                                setRenderPieChart("dept", seriesData, Highcharts);
                            }
                        } else if (id == "deptcol") {
                            if (val) {
                                let selected = vm.category_deptcol_stats[val];
                                ...
                                var top3Data = this.university_deptcol_tab_list;
                                if (top3Data.length > 3) {
                                    top3Data = this.university_deptcol_tab_list.slice(0, 3);
                                }
                            ...
                            if (val) {
                                ...
                                var top3Data = this.university_area_tab_list;
                                if (top3Data.length > 3) {
                                    top3Data = this.university_area_tab_list.slice(0, 3);
                                }
                        ...
                        if (null !== val) {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    if (0 < Object.keys(compobjs).length) {
                                        vm.companyListRelation = getSeriesDataByValue(compobjs);
                                    }
                                }
                            });
                        }
